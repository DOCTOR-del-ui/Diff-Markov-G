...
from Models.interpretable_diffusion.markov_utils import *
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self,
                 ...
                 mlp_hidden_times=4,
                 activate='GELU',
                 use_markovaware=False, 
                 use_resid_pdrop_markovaware=False,
                 resid_pdrop_markovaware=0.1
                 ):
        super().__init__()
        self.use_markovaware = use_markovaware
        ...
        if self.use_markovaware:
            self.markov_res = MarkovResidualBlock(d_model = n_embd, hidden_dim=None, scale=resid_pdrop_markovaware, use_resid_pdrop=use_resid_pdrop_markovaware)
    ...
    def forward(self, x, timestep, mask=None, label_emb=None):
        ...
        if self.use_markovaware:
            #print("编码器使用MarkovAware")
            x = x + self.markov_res(x)
        return x, att
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(*[EncoderBlock(
                ...
                use_markovaware=use_markovaware, 
                use_resid_pdrop_markovaware=use_resid_pdrop_markovaware,
                resid_pdrop_markovaware=resid_pdrop_markovaware
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.use_ff = use_ff
        self.use_markovhead = use_markovhead
        self.use_markovaware = use_markovaware
        ...
        if self.use_markovaware:
            self.markov_res = MarkovResidualBlock(d_model = n_embd, hidden_dim=None, 
                                                  scale=resid_pdrop_markovaware, 
                                                  use_resid_pdrop=use_resid_pdrop_markovaware)
    ...
    def forward(self, x, encoder_output, timestep, mask=None, label_emb=None):
        ...
        if self.use_markovaware:
            #print("解码器使用MarkovAware")
            x = x + self.markov_res(x)
        m = torch.mean(x, dim=1, keepdim=True)
        if self.use_ff:
            if self.use_markovhead:
                return x, x - m, self.linear(m), trend, season
            return x - m, self.linear(m), trend, season
        else:
            if self.use_markovhead:
                return x, x - m, self.linear(m), trend
...
class Decoder(nn.Module):
    def __init__(
      ...
      self.blocks = nn.Sequential(*[DecoderBlock(
                ...
                use_ff=use_ff,
                use_markovhead=use_markovhead,
                use_markovaware=use_markovaware, 
                use_resid_pdrop_markovaware=use_resid_pdrop_markovaware,
                resid_pdrop_markovaware=resid_pdrop_markovaware
        ) for _ in range(n_layer)])
      if self.use_markovhead:
        self.markov_head = MarkovHead(self.d_model, self.n_feat, seq_length=n_channel)
    ...
    def forward(self, x, t, enc, padding_masks=None, label_emb=None):
        ...
        for block_idx in range(len(self.blocks)):
            if self.use_ff:
                if self.use_markovhead:
                    h, x, residual_mean, residual_trend, residual_season = \
                ...
                if self.use_markovhead:
                    h, x, residual_mean, residual_trend = \
                        self.blocks[block_idx](x, enc, t, mask=padding_masks, label_emb=label_emb)
                else:
                    x, residual_mean, residual_trend = \
                        self.blocks[block_idx](x, enc, t, mask=padding_masks, label_emb=label_emb)
        ...
        if self.use_ff:
            """ print("x的形状:", x.shape)
            print("mean的形状:", mean.shape) """
            if self.use_markovhead:
                p_markov = self.markov_head(h)
                
                return x, mean, trend, season, p_markov
            return x, mean, trend, season
        else:
            if self.use_markovhead:
                p_markov = self.markov_head(h)
                return x, mean, trend, p_markov
            return x, mean, trend
...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.use_markovhead = use_markovhead
        self.use_markovaware = use_markovaware
        self.use_resid_pdrop_markovaware = use_resid_pdrop_markovaware
        self.use_maremb_weight = use_maremb_weight
        self.resid_pdrop_markovaware = resid_pdrop_markovaware
        self.maremb_weight = maremb_weight
        ...
        if self.use_markovaware:
            self.markov_enc = MarkovAwareEmbedding(n_feat, n_embd, scale=maremb_weight, 
                                                   use_maremb_weight=use_maremb_weight)

        self.decoder = Decoder(n_channel, n_feat, n_embd, n_heads, n_layer_dec, attn_pdrop, resid_pdrop, mlp_hidden_times,
                               block_activate, condition_dim=n_embd, use_ff=use_ff, use_markovhead=use_markovhead,
                               use_markovaware=use_markovaware, 
                               use_resid_pdrop_markovaware = use_resid_pdrop_markovaware,
                               resid_pdrop_markovaware=resid_pdrop_markovaware)
        self.pos_dec = LearnablePositionalEncoding(n_embd, dropout=resid_pdrop, max_len=max_len)
        if self.use_markovaware:
            self.markov_dec = MarkovAwareEmbedding(n_embd, n_embd, scale=maremb_weight, 
                                                   use_maremb_weight=use_maremb_weight)

    def forward(self, input, t, padding_masks=None, return_res=False):
        if self.use_markovaware:
            #print("编码器输入使用MarkovAware")
            emb = self.emb(input) + self.markov_enc(input)
        else:
            emb = self.emb(input)
        ...
        #print("enc_cond 形状:", enc_cond.shape)

        if self.use_markovaware:
            #print("解码器输入使用MarkovAware")
            enc_cond += self.markov_dec(enc_cond)
        inp_dec = self.pos_dec(emb) 
        #print("inp_dec 形状:", inp_dec.shape)
        if self.use_ff:
            if self.use_markovhead:
                output, mean, trend, season, p_markov = self.decoder(inp_dec, t, enc_cond, padding_masks=padding_masks)
            else:
                ...
                #print("trend 形状:", trend.shape)
                #print("season 形状:", season.shape)
        else:
            if self.use_markovhead:
                output, mean, trend, p_markov = self.decoder(inp_dec, t, enc_cond, padding_masks=padding_masks)
            else:
                output, mean, trend = self.decoder(inp_dec, t, enc_cond, padding_masks=padding_masks)

        res = self.inverse(output)
        res_m = torch.mean(res, dim=1, keepdim=True)
        
        #print("res 形状:", res.shape)
        #print("res_m 形状:", res_m.shape)
        
        if self.use_ff:
            season_error = self.combine_s(season.transpose(1, 2)).transpose(1, 2) + res - res_m
            #print("season_error 形状:", season_error.shape)
            trend = self.combine_m(mean) + res_m + trend
            #print("trend 形状:", trend.shape)            
            if return_res:
                if self.use_markovhead:
                    return trend, self.combine_s(season.transpose(1, 2)).transpose(1, 2), res - res_m, p_markov
                return trend, self.combine_s(season.transpose(1, 2)).transpose(1, 2), res - res_m
            if self.use_markovhead:
                return trend, season_error, p_markov
            return trend, season_error
        else:
            error = res
            trend = self.combine_m(mean) + trend + res_m
            if return_res:
                if self.use_markovhead:
                    return trend, res - res_m, p_markov
                return trend, res - res_m
            if self.use_markovhead:
                return trend, error, p_markov
            return trend, error
