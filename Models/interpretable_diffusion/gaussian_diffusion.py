...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        self.eta, self.use_ff, self.use_ffloss = eta, use_ff, use_ffloss
        self.use_markovloss = use_markovloss
        ...
        self.use_marloss_weight = use_marloss_weight
        self.resid_pdrop_markovaware = resid_pdrop_markovaware
        self.maremb_weight = maremb_weight
        self.marloss_weight = marloss_weight
        self.seq_length = seq_length
        self.feature_size = feature_size
        self.ff_weight = default(reg_weight, math.sqrt(self.seq_length) / 5)
        self.markov = MarkovTransition(num_states=128)
        ...
        self.model = Transformer(n_feat=feature_size, n_channel=seq_length, n_layer_enc=n_layer_enc, n_layer_dec=n_layer_dec,
                                 ...
                                 max_len=seq_length, n_embd=d_model, conv_params=[kernel_size, padding_size], use_ff=use_ff, 
        ...
        if self.use_marloss_weight:
            # 可学习权重
            self.marloss_weight_param = nn.Parameter(torch.tensor(self.marloss_weight, dtype=torch.float32))
        else:
            # 常数权重
            self.marloss_weight_param = None
    ...
    def output(self, x, t, padding_masks=None):
        if self.use_markovhead:
            trend, season, p_markov = self.model(x, t, padding_masks=padding_masks)
        else:
            trend, season = self.model(x, t, padding_masks=padding_masks)
        model_output = trend + season
        if self.use_markovhead:
            return model_output, p_markov
        
        #print("marloss_weight_param", self.marloss_weight_param)
    ...
    def model_predictions(self, x, t, clip_x_start=False, padding_masks=None):
        ...
        if self.use_markovhead:
            x_start, _ = self.output(x, t, padding_masks)
        else:
    ...
    def _train_loss(self, x_start, t, target=None, noise=None, padding_masks=None):
        ...
        if self.use_markovhead:
            model_out, p_markov = self.output(x, t, padding_masks)
        else:
            model_out = self.output(x, t, padding_masks)
        train_loss = self.loss_fn(model_out, target, reduction='none')
        
        #print("模型输出的最大最小值：", model_out.min(), model_out.max())
        #print("目标最大最小值：", target.min(), target.max())
        ...
        if self.use_ffloss:
            ...
            #print("傅里叶损失:", fourier_loss.min(), fourier_loss.max())
            train_loss +=  self.ff_weight * fourier_loss
        
        """ if self.use_markovloss:
            #print("模型输出形状:", model_out.shape)
            markov1 = self.markov(model_out)
            markov2 = self.markov(target)
            #print("目标马尔可夫", markov2.min(), markov2.max())
            markov_loss = self.loss_fn(markov1, markov2)  # scalar
            train_loss += markov_loss """
        
        train_loss = reduce(train_loss, 'b ... -> b (...)', 'mean')
        train_loss = train_loss * extract(self.loss_weight, t, train_loss.shape)
        train_loss = train_loss.mean()
        
        if self.use_markovloss and self.use_markovhead:
            markov_real = self.markov(target)
            markov_loss = self.loss_fn(p_markov, markov_real)  # scalar
            """ print("预测转移矩阵最大最小:", p_markov.min().item(), p_markov.max().item())
            print("真实转移矩阵最大最小:", markov_real.min().item(), markov_real.max().item())
            print("马尔可夫损失：", markov_loss.item()) """
            train_loss += markov_loss
        elif self.use_markovloss:
            #print("模型输出形状:", model_out.shape)
            markov1 = self.markov(model_out)
            markov2 = self.markov(target)
            #print("目标马尔可夫", markov2.min(), markov2.max())
            ...
            #print("马尔可夫损失值:", markov_loss, "原损失值:", train_loss, "马尔可夫形状:",markov1.shape)
            if self.marloss_weight_param is not None:
                # 可学习权重，映射到正数范围
                #print("当前马尔可夫损失权重参数值:")
                weight = F.softplus(self.marloss_weight_param)
            else:
                weight = self.marloss_weight   # 你设的固定权重（应>=0）
                
            train_loss += weight * markov_loss
    ...
    def langevin_fn(
        ...
        with torch.enable_grad():
            for i in range(K):
                ...
                optimizer.zero_grad()
                if self.use_markovhead:
                    x_start, _ = self.output(x=input_embs_param, t=t)
                else:
