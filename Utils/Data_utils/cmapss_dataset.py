...
class CmapssDataset(Dataset):
    def __init__(
        ...
        self.df, self.rawdata, self.scaler = self.read_data(data_root, self.name)
        self.dir = os.path.join(output_dir, 'samples')
        os.makedirs(self.dir, exist_ok=True)

        self.window, self.period = window, period
        self.len, self.var_num = self.rawdata.shape[0], len(feature_cols)
    ...
    def __getsamples(self, data, proportion, seed):
        ...
        indexs = []
        for id in ids:
            curr_index = self.df[self.df['id'] == id].index
            sample_num_total += len(curr_index) - self.window + 1
            indexs.append(curr_index)
            
        x = np.zeros((sample_num_total, self.window, self.var_num))
        i = 0
        for curr_index in indexs:
            curr_sample_num = len(curr_index) - self.window + 1
            for start in range(curr_index[0], curr_index[0] + curr_sample_num):
